...
| Skill | 用途 | 大小 |
|-------|------|------|
| `frontend-design-ultra` | 前端设计总监技能：按页面形态分流设计流程（工作台/数据大屏/移动 H5/落地页及扩展形态），方向合同→任务布局决策→设计计划→反俗套分级→规范构建→分级验证，专治 AI 模板感 | ~310 KB（38 文件） |
...
资源与可靠性条款（经三轮独立代理试跑校验）：中文正文默认系统字体栈，只有标题、主视觉、品牌位确需设计感时才引 CDN 字体且只声明 1–2 档；图标只走 CDN 加载式方案（Tabler 图标字体或 Lucide 运行时），不手写、不手绘、不抄 path、不自建精灵图；外部资源加载判定依据明确错误，不用固定短超时把网络慢判成失败；状态点禁用循环闪烁与呼吸，循环动效只服务加载与刷新过程。
