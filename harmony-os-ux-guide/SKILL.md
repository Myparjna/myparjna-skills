...
- **通用设计默认入口**：01 设计原则、02 的色彩/排版/间距/圆角/间隔/沉浸光感、03–08 组件设计、09 响应式布局、10 交互、11 多设备、15 输入设备、16 动效、17 行业案例。提取信息层级、可读性、反馈、布局与可访问性方法，再适配目标平台。
- **仅对应平台时加载**：13 元服务、14 系统特性（实况窗、锁屏通知、服务卡片、状态栏、闪控球、平行视界、智感握姿等）、19 智能座舱、12 的元服务标准、02 的 HarmonyOS Symbol，以及 18 中华为账号/支付、NFC碰一碰、近场发现、智慧握持等。用户明确开发 HarmonyOS 或要求比较平台差异时再读取；网页或 iOS 的类似能力应另查对应平台文档，不直接套用鸿蒙能力或 API。
- **混合章节**：12 UX标准、18 补充内容及其他章节中的系统能力，只保留可迁移的设计思路，输出中分别标注"通用建议"和"HarmonyOS 专用"。未指定平台时默认通用，不主动引入系统集成。
- **大文件按需读取**：`references/02-visual-design/harmonyos-symbol-motion-table.html` 为旧版动效策略原始长表格的无损抽离件（2026 新版正文已不再内嵌该表）。只在核对动效素材时用浏览器查看该表格，禁止全文读取内嵌资源。
...
色彩体系、字体排版、鸿蒙黑体、间距规范、深色模式、HarmonyOS Symbol图标（2026 更新）、视觉风格、留白规范，及 2026 新增：沉浸光感（材质分级与场景规范）、圆角参数、间隔参数。
- `color.md` / `typography.md` / `harmonyos-font.md` / `spacing.md` / `dark-mode.md` / `harmonyos-symbol.md` / `visual-style.md` / `blank.md` / `immersive-light.md` / `corner-radius.md` / `interval-parameter.md`

### 03 - 操作类组件 (`references/03-components-action/`)
按钮、芯片、菜单、分段按钮、选择按钮、切换按钮等操作型组件的使用规范。
- `action-overview.md`（2026 更新）/ `button.md` / `chips.md` / `menu.md` / `segment-button.md` / `select-button.md` / `toggle-button.md`

### 04 - 输入类组件 (`references/04-components-input/`)
复选框、计数器、输入框、选择器、单选按钮、滑块、文本输入、开关等输入型组件，及 2026 新增色彩选择器。
- `input-overview.md`（2026 更新）/ `selection-overview.md`（2026 新增，官方已将"选择类"从输入类拆分为独立分类）/ `checkbox.md` / `counter.md` / `picker.md` / `color-picker.md` / `radio.md` / `slider.md` / `text-input.md` / `toggle-switch.md`

### 05 - 展示类组件 (`references/05-components-display/`)
字母索引、徽标、数据面板、分割线、图片、列表、进度条、评分、滚动条、文本等展示型组件。
- `display-overview.md`（2026 更新）/ `alphabet-indexer.md` / `badge.md` / `data-panel.md` / `divider.md` / `image.md` / `list.md` / `progress.md` / `rating.md` / `scrollbar.md` / `text.md`

### 06 - 导航类组件 (`references/06-components-navigation/`)
底部Tab、导航栏、子Tabs、滑动指示点、标题栏、工具栏等导航型组件。
- `navigation-overview.md`（2026 更新）/ `bottom-tab.md` / `navigation-bar.md` / `sub-tabs.md` / `swiper-dots.md` / `titlebar.md` / `toolbar.md`

### 07 - 容器类组件 (`references/07-components-container/`)
对话框、半屏弹窗、弹出层等容器型组件。
- `container-overview.md`（2026 更新）/ `dialog.md` / `half-sheet.md` / `popup.md`
...
通用交互、光标、拖拽、焦点导航、手势（2026 新增隔空手势）、人机交互概览、交互事件归一（2026 更新，含各输入设备参数）、选择等交互规范。
...
手机（2026 更新）、平板、折叠屏（含双折/三折/宽折）、PC、智慧屏（2026 更新）、穿戴等设备形态的设计要点。智能座舱见 19。
...
通用UX标准（2026 更新，新增可滑动控件高度、画中画适配、滑动沉浸、短视频沉浸）、折叠屏标准（2026 新增标题栏/搜索框/索引条/底部页签适配）、大屏标准（2026 更新）、PC标准（2026 更新）、智慧屏标准、穿戴标准（2025-11 官方新版）、元服务标准。
...
启动页、实况窗、多窗口、通知、画中画、服务卡片、状态栏、系统能力等系统级特性，及 2026 新增：闪控球和闪控窗、桌面快捷方式、平行视界、智感握姿。
- `system-features.md` / `launch-page.md` / `live-view.md` / `multi-window.md` / `notification.md` / `pip.md` / `service-widget-system.md` / `statusbar.md` / `system-capabilities.md` / `float-view.md` / `desktop-shortcut.md` / `parallel-view.md` / `smart-reachability.md`
...
汽车、相机、日常生活、电商、娱乐、金融、游戏、新闻、办公、社交、文旅、出行、移动支付（2026 新增）等行业的响应式设计案例，2026 年批次已全面换新为新材质组件版本。
- `responsive-overview.md` / `automotive.md` / `camera.md` / `daily-life.md` / `ecommerce.md` / `entertainment.md` / `finance.md` / `game.md` / `news.md` / `office.md` / `social.md` / `tourism.md` / `travel.md` / `mobile-payment.md`

### 18 - 补充内容 (`references/18-supplementary/`)
账号、应用设计、应用图标（2026 更新）、最佳实践、广播控制、变更说明（2026 更新至 2026-06 HDC 版本）、组件概览（2026 更新）、全球化、华为账号、华为支付、近场发现、NFC碰一碰、图案锁、隐私、二维码、智慧握持、文字时钟、UI语言、UX标准概览、穿戴概览等。
...
### 19 - 智能座舱（HarmonyOS 专用，按需加载） (`references/19-smart-cockpit/`)
2026 新增专题。智能座舱设计概述、基础体验要求、设计基础、系统架构、应用架构、HUAWEI HiCar。
- `smart-cockpit.md`（目录页）/ `cockpit-overview.md` / `basic-experience.md` / `ux-basic.md` / `system-architecture.md` / `application-architecture.md` / `hicar.md`
...
| 车载/智能座舱场景 | `19-smart-cockpit/cockpit-overview.md` |
| 隔空手势等非接触交互 | `10-interaction/gesture.md` |
| 沉浸光感材质与档位选择 | `02-visual-design/immersive-light.md` |
| 查看最新官方变更 | `18-supplementary/changelog.md` |

## 数据来源

所有内容抓取自 `developer.huawei.com/consumer/cn/doc/design-guides/`。首批抓取时间 2026-04-04；2026-09-14 增量更新 49 篇（33 篇官方 2026 更新批次 + 16 篇 2026 新增补抓，含 2026-06 HDC 版本的沉浸光感、圆角参数、间隔参数、鸿蒙黑体、色彩选择器、闪控球、桌面快捷方式、平行视界、智感握姿、智能座舱专题与穿戴 UX 标准），各文件头部标注了官方页面更新时间。
