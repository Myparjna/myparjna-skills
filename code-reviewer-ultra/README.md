...
评审聚焦 bug、正确性及有实际影响的安全、性能、可靠性回归；命名风格、架构偏好、缺测试本身不作为独立 bug。默认只读，保留明确 review-and-fix 授权入口。按影响判断小改动，先读 diff、相关函数及必要调用和测试，证据充分不通读全文件；公共契约、安全并发、迁移再展开。小改动简报保留发现、验证实际结果和限制，完整模板按需使用。下列历史升级及来源记录保留，当前执行规则以 SKILL.md 为准。
